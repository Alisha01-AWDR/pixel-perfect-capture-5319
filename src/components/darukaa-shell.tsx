...
export function AuthPage({ mode }: { mode: "login" | "register" }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) { event.preventDefault(); setError(""); setIsBusy(true); try { if (mode === "login") await login(email, password); else await register(email, password); navigate({ to: "/dashboard" }); } catch { setError("We couldn't complete that request. Check your details and try again."); } finally { setIsBusy(false); } }
